...
14. Set -write_obj_id to output a copy of the input file with an 'obj_id' column containing the coordinate-based IDs for each posted object. Use this file as input for future uploads to add to this column.
15. Set -result_dir to customize the directory where the upload results file is saved. Default is 'fritzUpload' within the tools directory.
16. Set -result_filetag to customize the tag appended to the result filename. Default is 'fritzUpload'.
17. Set -result_format to one of csv, h5 or parquet to customize the result file format. Default is parquet.
