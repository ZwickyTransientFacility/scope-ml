...
8. Set -post_survey_id to post an annotation for the Gaia, AllWISE or PS1 id associated with each source
9. Annotation origin name for survey_id
...
12. Set -match_ids to match input and existing survey_id values during upload. It is recommended to instead match obj_ids (see next line)
13. Set -use_existing_obj_id to provide existing source names in a column named 'obj_id' (a coordinate-based ID is otherwise generated by default)
14. Set -post_upvote to post an upvote to newly uploaded classifications. Not recommended when posting automated classifications for active learning.
15. Set -check_labelled_box to check the 'labelled' box for each source when uploading classifications. Not recommended when posting automated classifications for active learning.
16. Set -write_obj_id to output a copy of the input file with an 'obj_id' column containing the coordinate-based IDs for each posted object. Use this file as input for future uploads to add to this column.
17. Set -result_dir to customize the directory where the upload results file is saved. Default is 'fritzUpload' within the tools directory.
18. Set -result_filetag to customize the tag appended to the result filename. Default is 'fritzUpload'.
19. Set -result_format to one of csv, h5 or parquet to customize the result file format. Default is parquet.
20. Set -replace_classifications to delete each source's existing classifications before posting new ones.
